...
- **services** — список geo-сервисов, используемых для определения местоположения по IP. Каждый провайдер поддерживает только свои параметры (см. таблицу ниже).
...
В запросе будут использоваться только параметры, указанные в `requiredParams` и `optionalParams` для каждого провайдера. Если обязательный параметр не задан, будет выброшена и залогирована ошибка. Необязательные параметры добавляются только если заданы в конфиге.
